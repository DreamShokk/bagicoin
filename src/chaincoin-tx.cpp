...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " chaincoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  chaincoin-tx [options] <hex-tx> [commands]  Update hex-encoded chaincoin transaction\n" +
            "or:     chaincoin-tx [options] -create [commands]   Create hex-encoded chaincoin transaction\n" +
            "\n";
