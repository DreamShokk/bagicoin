...
void SetupChainParamsBaseOptions()
{
    ...
    gArgs.AddArg("-segwitheight=<n>", "Set the activation height of segwit. -1 to disable. (regtest-only)", true, OptionsCategory::DEBUG_TEST);
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const std::string& chain)
{
    if (chain == CBaseChainParams::MAIN)
        ...
        return MakeUnique<CBaseChainParams>("testnet4", 21995);
