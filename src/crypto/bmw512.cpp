...
void inline Initialize(bmw_context *sc, const sph_u64 *iv)
{
...
void inline compress(const unsigned char *data, const sph_u64 h[16], sph_u64 dh[16])
{
    sph_u64 mv[16];
...
CBMW512::CBMW512()
{
    bmw512::Initialize(&s, IV512);
}

CBMW512& CBMW512::Write(const unsigned char* data, size_t len)
{
    unsigned char *buf;
    size_t ptr;
    sph_u64 htmp[16];
    ...
    s.ptr = ptr;
    if (h1 != s.H)
        memcpy(s.H, h1, sizeof s.H);
    return *this;
}

void CBMW512::Finalize(unsigned char hash[OUTPUT_SIZE])
{
    unsigned char *buf;
    size_t ptr, u, v;
    unsigned z;
    ...
    buf[ptr ++] = ((0 & -z) | z) & 0xFF;
    h = s.H;
    if (ptr > (sizeof s.buf) - 8) {
        memset(buf + ptr, 0, (sizeof s.buf) - ptr);
        bmw512::compress(buf, h, h1);
        ptr = 0;
        h = h1;
    }
    memset(buf + ptr, 0, (sizeof s.buf) - 8 - ptr);
    sph_enc64le_aligned(buf + (sizeof s.buf) - 8,
        SPH_T64(s.bit_count));
    bmw512::compress(buf, h, h2);
    for (u = 0; u < 16; u ++)
        sph_enc64le_aligned(buf + 8 * u, h2[u]);
    bmw512::compress(buf, final_b, h1);
    for (u = 0, v = 8; u < 8; u ++, v ++)
        sph_enc64le(hash + 8 * u, h1[v]);
    Reset();
}

CBMW512& CBMW512::Reset()
{
    bmw512::Initialize(&s, IV512);
    return *this;
}
