...
#define READ_STATE_BIG(sc)   do { \
        memcpy(H, sc.state, sizeof H); \
    } while (0)

#define WRITE_STATE_BIG(sc)   do { \
        memcpy(sc.state, H, sizeof H); \
    } while (0)
...
namespace
{
/// Internal GROESTL512 implementation.
namespace groestl512
{

void inline Initialize(groestl_context *sc, unsigned out_size)
{
    sc->ptr = 0;
    for (int i = 0; i < 15; i ++)
        sc->state[i] = 0;
#if USE_LE
    sc->state[15] = ((sph_u64)(out_size & 0xFF) << 56)
        | ((sph_u64)(out_size & 0xFF00) << 40);
#else
    sc->state[15] = (sph_u64)out_size;
#endif
    sc->count = 0;
}
...
CGROESTL512::CGROESTL512()
{
    groestl512::Initialize(&s, 512);
}

CGROESTL512& CGROESTL512::Write(const unsigned char* data, size_t len)
{
    unsigned char *buf;
    size_t ptr;
    DECL_STATE_BIG
...
void CGROESTL512::Finalize(unsigned char hash[OUTPUT_SIZE])
{
    ...
    if (ptr < 120) {
        pad_len = 128 - ptr;
        count = SPH_T64(s.count + 1);
    } else {
        pad_len = 256 - ptr;
        count = SPH_T64(s.count + 2);
    }
