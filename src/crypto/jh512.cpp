...
namespace
{
/// Internal JH512 implementation.
namespace jh512
{

void inline Initialize(jh_context *sc, const void *iv)
{
    sc->ptr = 0;
...
CJH512::CJH512()
{
    jh512::Initialize(&s, IV512);
}

CJH512& CJH512::Write(const unsigned char* data, size_t len)
{
    unsigned char *buf;
    size_t ptr;
    DECL_STATE
...
void CJH512::Finalize(unsigned char hash[OUTPUT_SIZE])
{
    unsigned z;
    ...
    memset(buf + 1, 0, numz);
    l0 = SPH_T64(s.block_count << 9) + (s.ptr << 3);
    l1 = SPH_T64(s.block_count >> 55);
    sph_enc64be(buf + numz + 1, l1);
    sph_enc64be(buf + numz + 9, l0);
    Write(buf, numz + 17);
    for (u = 0; u < 8; u ++)
        enc64e(buf + (u << 3), s.H[u + 8]);
    memcpy(hash, buf + (0 << 2), 16 << 2);
    Reset();
}
