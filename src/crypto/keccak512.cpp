...
#include <string.h>
    ...
    /*
     * We do not want to copy the _state to local variables on x86 (32-bit
     * and 64-bit alike).
     */
...
#define a00   (s.state[ 0])
#define a10   (s.state[ 1])
#define a20   (s.state[ 2])
#define a30   (s.state[ 3])
#define a40   (s.state[ 4])
#define a01   (s.state[ 5])
#define a11   (s.state[ 6])
#define a21   (s.state[ 7])
#define a31   (s.state[ 8])
#define a41   (s.state[ 9])
#define a02   (s.state[10])
#define a12   (s.state[11])
#define a22   (s.state[12])
#define a32   (s.state[13])
#define a42   (s.state[14])
#define a03   (s.state[15])
#define a13   (s.state[16])
#define a23   (s.state[17])
#define a33   (s.state[18])
#define a43   (s.state[19])
#define a04   (s.state[20])
#define a14   (s.state[21])
#define a24   (s.state[22])
#define a34   (s.state[23])
#define a44   (s.state[24])
...
#define INPUT_BUF(size)   do { \
        size_t j; \
        for (j = 0; j < (size); j += 8) { \
            s.state[j >> 3] ^= sph_dec64le_aligned(buf + j); \
        } \
    } while (0)
...
namespace
{
/// Internal KECCAK512 implementation.
namespace keccak512
{

void inline Initialize(keccak_context* sc, unsigned out_size)
{
    for (int i = 0; i < 25; i ++)
        sc->state[i] = 0;
    /*
     ...
     */
    sc->state[ 1] = SPH_C64(0xFFFFFFFFFFFFFFFF);
    sc->state[ 2] = SPH_C64(0xFFFFFFFFFFFFFFFF);
    sc->state[ 8] = SPH_C64(0xFFFFFFFFFFFFFFFF);
    sc->state[12] = SPH_C64(0xFFFFFFFFFFFFFFFF);
    sc->state[17] = SPH_C64(0xFFFFFFFFFFFFFFFF);
    sc->state[20] = SPH_C64(0xFFFFFFFFFFFFFFFF);
    sc->ptr = 0;
    sc->lim = 200 - (out_size >> 2);
}
...
CKECCAK512::CKECCAK512()
{
    keccak512::Initialize(&s, 512);
}

CKECCAK512& CKECCAK512::Write(const unsigned char* data, size_t len)
{
    unsigned char *buf;
    size_t ptr;
    DECL_STATE
...
void CKECCAK512::Finalize(unsigned char hash[OUTPUT_SIZE])
{
    unsigned eb;
    union {
    ...
    s.state[ 2] = ~s.state[ 2];
    s.state[ 8] = ~s.state[ 8];
    s.state[12] = ~s.state[12];
    s.state[17] = ~s.state[17];
    s.state[20] = ~s.state[20];
    for (j = 0; j < 64; j += 8)
        sph_enc64le_aligned(u.tmp + j, s.state[j >> 3]);
    memcpy(hash, u.tmp, 64);
    Reset();
}

CKECCAK512& CKECCAK512::Reset()
{
    keccak512::Initialize(&s, 512);
    return *this;
}
