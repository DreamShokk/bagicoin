...
namespace
{
/// Internal SHAVITE512 implementation.
namespace shavite512
{

void inline Initialize(shavite_context *sc, const sph_u32 *iv)
{
...
CSHAVITE512::CSHAVITE512()
{
    shavite512::Initialize(&s, IV512);
}

CSHAVITE512& CSHAVITE512::Write(const unsigned char* data, size_t len)
{
    unsigned char *buf;
    size_t ptr;
    ...
    while (len > 0) {
        ...
        len -= clen;
        if (ptr == sizeof s.buf) {
            if ((s.count0 = SPH_T32(s.count0 + 1024)) == 0) {
                s.count1 = SPH_T32(s.count1 + 1);
                if (s.count1 == 0) {
                    s.count2 = SPH_T32(s.count2 + 1);
                    if (s.count2 == 0) {
                        s.count3 = SPH_T32(
                            s.count3 + 1);
                    }
...
void CSHAVITE512::Finalize(unsigned char hash[OUTPUT_SIZE])
{
    unsigned char *buf;
    size_t ptr, u;
    unsigned z;
    ...
    if (ptr == 0) {
        buf[0] = 0x80;
        memset(buf + 1, 0, 109);
        s.count0 = s.count1 = s.count2 = s.count3 = 0;
    } else if (ptr < 110) {
        buf[ptr ++] = z;
    ...
    c512(&s, buf);
    for (u = 0; u < 16; u ++)
        sph_enc32le(hash + (u << 2), s.h[u]);
    Reset();
}

CSHAVITE512& CSHAVITE512::Reset()
{
    shavite512::Initialize(&s, IV512);
    return *this;
}
