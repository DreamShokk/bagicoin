...
namespace
{
/// Internal SIMD512 implementation.
namespace simd512
{

void inline Initialize(simd_context *sc, const u32 *iv)
{
...
void inline encode_count(unsigned char *dst,
        u32 low, sph_u32 high, size_t ptr)
{
        low = T32(low << 10);
...
void inline compress(simd_context *sc, int last)
{
        unsigned char *x;
        s32 q[256];
        int i;
...
} // namespace shavite512

} // namespace

CSIMD512::CSIMD512()
{
    simd512::Initialize(&s, IV512);
}

CSIMD512& CSIMD512::Write(const unsigned char* data, size_t len)
{
    while (len > 0) {
        size_t clen;

        clen = (sizeof s.buf) - s.ptr;
        if (clen > len)
            clen = len;
        memcpy(s.buf + s.ptr, data, clen);
        data = (const unsigned char *)data + clen;
        len -= clen;
        if ((s.ptr += clen) == sizeof s.buf) {
            simd512::compress(&s, 0);
            s.ptr = 0;
            s.count_low = T32(s.count_low + 1);
            if (s.count_low == 0)
                s.count_high ++;
        }
    }
...
void CSIMD512::Finalize(unsigned char hash[OUTPUT_SIZE])
{
    if (s.ptr > 0) {
        memset(s.buf + s.ptr, 0,
               (sizeof s.buf) - s.ptr);
        s.buf[s.ptr] = 0 & (0xFF << (8));
        simd512::compress(&s, 0);
    }
    memset(s.buf, 0, sizeof s.buf);
    simd512::encode_count(s.buf, s.count_low, s.count_high, s.ptr);
    simd512::compress(&s, 1);
    for (int u = 0; u < 16; u ++)
        sph_enc32le(hash + (u << 2), s.state[u]);
    Reset();
}

CSIMD512& CSIMD512::Reset()
{
    simd512::Initialize(&s, IV512);
    return *this;
}
