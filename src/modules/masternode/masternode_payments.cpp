...
*
*   Why is this needed?
*   - In Chaincoin some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string& strErrorRet)
{
    ...
    LogPrint(BCLog::MNODE, "block.vtx[0]->GetValueOut() %lld <= blockReward %lld\n", block.vtx[0]->GetValueOut(), blockReward);
    ...
    LogPrint(BCLog::GOV, "block.vtx[0]->GetValueOut() %lld <= nSuperblockMaxValue %lld\n", block.vtx[0]->GetValueOut(), nSuperblockMaxValue);

    if (!masternodeSync.IsSynced() || fLiteMode) {
        // not enough data but at least it must NOT exceed superblock max value
        if (CSuperblock::IsValidBlockHeight(nBlockHeight)) {
            LogPrint(BCLog::MNODESYNC, "IsBlockPayeeValid -- Not enough data, checking superblock max bounds only\n");
    ...
    if (nBlockHeight >= consensusParams.nSuperblockStartBlock) {
        ...
        LogPrint(BCLog::GOV, "IsBlockValueValid -- No triggered superblock detected at height %d\n", nBlockHeight);
        if (!isBlockRewardValueMet) {
        ...
        LogPrint(BCLog::GOV, "IsBlockValueValid -- Superblocks are disabled, no superblocks allowed\n");
        if (!isBlockRewardValueMet) {
...
bool IsBlockPayeeValid(const CTransactionRef& txNew, int nBlockHeight, CAmount blockReward)
{
    if (!masternodeSync.IsSynced() || fLiteMode) {
        ...
        LogPrint(BCLog::MNODESYNC, "IsBlockPayeeValid -- WARNING: Not enough data, skipping block payee checks\n");
        return true;
    }
    ...
    if (nBlockHeight >= consensusParams.nSuperblockStartBlock) {
        ...
        LogPrint(BCLog::GOV, "IsBlockPayeeValid -- No triggered superblock detected at height %d\n", nBlockHeight);
    } else {
        // should NOT allow superblocks at all, when superblocks are disabled
        LogPrint(BCLog::GOV, "IsBlockPayeeValid -- Superblocks are disabled, no superblocks allowed\n");
    }
...
{
    // only create superblocks if superblocks are enabled AND if superblock is actually triggered
    // (height should be validated inside)
    const Consensus::Params& consensusParams = Params().GetConsensus();
    if ((nBlockHeight >= consensusParams.nSuperblockStartBlock) &&
        CSuperblockManager::IsSuperblockTriggered(nBlockHeight)) {
            LogPrint(BCLog::GOV, "FillBlockPayments -- triggered superblock creation at height %d\n", nBlockHeight);
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet) const
{
    ...
    if (!GetBlockPayee(nBlockHeight, payee)) {
        ...
        payee = GetScriptForDestination(mnInfo.collDest);
    }
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
...

        {
            ...
            if (!res.second && res.first->second.IsVerified()) {
                LogPrint(BCLog::MNODEPAY, "MASTERNODEPAYMENTVOTE -- hash=%s, nBlockHeight=%d/%d seen\n",
        ...
        if (vote.nBlockHeight < nFirstBlock || vote.nBlockHeight > nCachedBlockHeight+20) {
            LogPrint(BCLog::MNODEPAY, "MASTERNODEPAYMENTVOTE -- vote out of range: nFirstBlock=%d, nBlockHeight=%d, nHeight=%d\n", nFirstBlock, vote.nBlockHeight, nCachedBlockHeight);
            return;
        }

        std::string strError = "";
        if (!vote.IsValid(pfrom, nCachedBlockHeight, strError, connman)) {
            LogPrint(BCLog::MNODEPAY, "MASTERNODEPAYMENTVOTE -- invalid message, error: %s\n", strError);
            return;
        }
        ...
        if (!vote.CheckSignature(mnInfo.pubKeyMasternode, nCachedBlockHeight, nDos)) {
            if (nDos) {
                ...
                LogPrint(BCLog::MNODEPAY, "MASTERNODEPAYMENTVOTE -- WARNING: invalid signature\n");
            }
...
bool CMasternodePayments::IsScheduled(const masternode_info_t& mnInfo, int nNotBlockHeight) const
{
    ...
    CScript mnpayee;
    mnpayee = GetScriptForDestination(mnInfo.collDest);
...
bool CMasternodePayments::AddOrUpdatePaymentVote(const CMasternodePaymentVote& vote)
{
    ...
    if (!mnodeman.HasBlockHash(blockHash, vote.nBlockHeight - 101)) return false;
    ...
    LogPrint(BCLog::MNODEPAY, "CMasternodePayments::AddOrUpdatePaymentVote -- added, hash=%s\n", nVoteHash.ToString());
...
bool CMasternodeBlockPayees::GetBestPayee(CScript& payeeRet) const
{
    ...
    if (vecPayees.empty()) {
        LogPrint(BCLog::MNODEPAY, "CMasternodeBlockPayees::GetBestPayee -- ERROR: couldn't find any payee\n");
        return false;
    }
...
bool CMasternodeBlockPayees::HasPayeeWithVotes(const CScript& payeeIn, int nVotesReq) const
{
    ...
    LogPrint(BCLog::MNODEPAY, "CMasternodeBlockPayees::HasPayeeWithVotes -- ERROR: couldn't find any payee with %d+ votes\n", nVotesReq);
    return false;
}
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransactionRef& txNew) const
{
        ...
        if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            for (const auto& txout : txNew->vout) {
                if (payee.GetPayee() == txout.scriptPubKey && nMasternodePayment == txout.nValue) {
                    LogPrint(BCLog::MNODEPAY, "CMasternodeBlockPayees::IsTransactionValid -- Found required payment\n");
                    return true;
                }
            }
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f CHC\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
void CMasternodePayments::CheckAndRemove()
{
    ...
    while(it != mapMasternodePaymentVotes.end()) {
        ...
        if (nCachedBlockHeight - vote.nBlockHeight > nLimit) {
            LogPrint(BCLog::MNODEPAY, "CMasternodePayments::CheckAndRemove -- Removing old Masternode payment: nBlockHeight=%d\n", vote.nBlockHeight);
    ...
    LogPrint(BCLog::MNODEPAY, "CMasternodePayments::CheckAndRemove -- %s\n", ToString());
}
...
bool CMasternodePaymentVote::IsValid(CNode* pnode, int nValidationHeight, std::string& strError, CConnman* connman) const
{
    ...
    if (mnInfo.nProtocolVersion < MIN_MASTERNODE_PAYMENT_PROTO_VERSION) {
        strError = strprintf("Masternode protocol is too old: nProtocolVersion=%d, nMinRequiredProtocol=%d", mnInfo.nProtocolVersion, MIN_MASTERNODE_PAYMENT_PROTO_VERSION);
        return false;
    }
    ...
    if (!mnodeman.GetMasternodeRank(masternodeOutpoint, nRank, nBlockHeight - 101, MIN_MASTERNODE_PAYMENT_PROTO_VERSION)) {
        LogPrint(BCLog::MNODEPAY, "CMasternodePaymentVote::IsValid -- Can't calculate rank for masternode %s\n",
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman* connman)
{
    ...
    if (!mnodeman.GetMasternodeRank(activeMasternode.outpoint, nRank, nBlockHeight - 101, GetMinMasternodePaymentsProto())) {
        LogPrint(BCLog::MNODEPAY, "CMasternodePayments::ProcessBlock -- Unknown Masternode\n");
        return false;
    }

    if (nRank > MNPAYMENTS_SIGNATURES_TOTAL) {
        LogPrint(BCLog::MNODEPAY, "CMasternodePayments::ProcessBlock -- Masternode not in the top %d (%d)\n", MNPAYMENTS_SIGNATURES_TOTAL, nRank);
        return false;
    }
    ...
    CScript payee = GetScriptForDestination(mnInfo.collDest);
...
void CMasternodePaymentVote::Relay(CConnman* connman) const
{
    ...
    if (!masternodeSync.IsSynced()) {
        LogPrint(BCLog::MNODEPAY, "CMasternodePayments::Relay -- won't relay until fully synced\n");
        return;
    }
