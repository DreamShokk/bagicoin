...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in bagicoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in bagicoin.qrc)
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // bagicoin: URIs or payment requests:
        if (paymentServer) {
...
{
    QMessageBox::critical(nullptr, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Bagicoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bagicoin);
    Q_INIT_RESOURCE(bagicoin_locale);
    ...
    /// 6. Determine availability of data directory and parse bagicoin.conf
    ...
    // bagicoin: links repeatedly have their payment requests routed to this process:
