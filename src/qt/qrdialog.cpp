...
#include <config/bagicoin-config.h> /* for USE_QRCODE */
#endif
...
void QRDialog::setModel(OptionsModel *_model)
{
    this->model = _model;

    if (_model)
        connect(_model, &OptionsModel::displayUnitChanged, this, &QRDialog::update);
