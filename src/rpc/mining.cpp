...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address    (string, required) The address to send the newly generated chaincoin to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when chaincoind is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    result.push_back(Pair("superblocks_enabled", pindexPrev->nHeight + 1 > Params().GetConsensus().nSuperblockStartBlock));
