...
static UniValue getpeerinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"subver\": \"/Chaincoin Core:x.x.x/\",  (string) The string version\n"
    ...
    if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3)
        throw std::runtime_error(
            RPCHelpMan{"disconnectnode",
                ...
                .ToString() +
            "\nExamples:\n"
            + HelpExampleCli("disconnectnode", "\"192.168.0.6:11994\"")
            + HelpExampleCli("disconnectnode", "\"\" 1")
            + HelpExampleRpc("disconnectnode", "\"192.168.0.6:11994\"")
...
static UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:11994\",  (string) The chaincoin server IP and port we're connected to\n"
...
static UniValue getnetworkinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"subversion\": \"/Chaincoin Core:x.x.x/\",     (string) the server subversion string\n"
