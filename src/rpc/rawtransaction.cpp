...
static UniValue getrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "           \"chaincoinaddress\"      (string) chaincoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
static UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"CwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\"   (string) chaincoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"decodepsct",
                ...
                {
                    {"psbt", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The PSCT base64 string"},
                },
                RPCResult{
            "{\n"
            ...
            "          \"hex\" : \"hex\",            (string) The hex\n"
            "          \"type\" : \"pubkeyhash\",    (string) The type, eg 'pubkeyhash'\n"
            "          \"address\" : \"address\"     (string) Chaincoin address if there is one\n"
            "        }\n"
            "      },\n"
            ...
            RPCHelpMan{"combinepsct",
                ...
                "Implements the Combiner role.\n",
                {
                    {"txs", RPCArg::Type::ARR, /* opt */ false, /* default_val */ "", "A json array of base64 strings of partially signed transactions",
                        {
                            {"psct", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "A base64 string of a PSBT"},
                        },
                        },
                },
                RPCResult{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"finalizepsct",
                ...
                {
                    {"psct", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "A base64 string of a PSCT"},
