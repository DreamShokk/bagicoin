...
/* Define a virtual block time, one block per 1.5 minutes after Nov 14 2014, 0:55:36am */
static int32_t TestTime(int nHeight) { return 1415926536 + 90 * nHeight; }
...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    lastBlock = firstChain.Mine(13440, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);

    // Mine 2011 more blocks at the old time, and check that CBV isn't setting the bit yet.
    for (int i=1; i<13436; i++) {
        lastBlock = firstChain.Mine(13440+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime = nStartTime;
    for (int i=13436; i<=13440; i++) {
        lastBlock = firstChain.Mine(13440+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
    }

    // Advance to the next period and transition to STARTED,
    lastBlock = firstChain.Mine(40320, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime += 90;
    int blocksToMine = 26880; // test blocks for up to 2 time periods
    int nHeight = 40320;
    ...
    // next period.
    lastBlock = secondChain.Mine(13440, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);

    // Mine another period worth of blocks, signaling the new bit.
    lastBlock = secondChain.Mine(26880, nTime, VERSIONBITS_TOP_BITS | (1<<bit)).Tip();
    ...
    lastBlock = secondChain.Mine(40319, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
    lastBlock = secondChain.Mine(40320, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
