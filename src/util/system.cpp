...
const char * const CHAINCOIN_CONF_FILENAME = "bagicoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bagicoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bagicoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bagicoin
    // Mac: ~/Library/Application Support/Bagicoin
    // Unix: ~/.bagicoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bagicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bagicoin";
#else
    // Unix
    return pathRet / ".bagicoincore";
#endif
#endif
}
