...
void CCoinJoinClientManager::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman* connman)
{
    ...
    if (strCommand == NetMsgType::CJQUEUE) {
        ...
        if (!mnodeman.GetMasternodeInfo(queue.masternodeOutpoint, infoMn)) return;

        if (!queue.CheckSignature(infoMn.pubKeyMasternode)) {
...
bool CCoinJoinClientSession::PoolStateManager()
{
    ...
    case POOL_STATE_QUEUE:
    case POOL_STATE_ACCEPTING_ENTRIES:
        if (GetTime() - nTimeLastSuccessfulStep > COINJOIN_QUEUE_TIMEOUT) {
            LogPrint(BCLog::CJOIN, "%s CCoinJoinClientSession::CheckTimeout -- Queue timed out -- resetting\n", m_wallet_session->GetDisplayName());
            SetNull();
        ...
        return false;
    case POOL_STATE_SIGNING:
        if (GetTime() - nTimeLastSuccessfulStep > COINJOIN_SIGNING_TIMEOUT + 10) {
...
bool CCoinJoinClientManager::CreateDenominated(const CAmount& nValue, bool fOnlyFees)
{
    ...
    // small denoms for fees first, then check for the highest we can cover at the requested value
    CAmount nDenomValue = COINJOIN_LOW_DENOM;

    LogPrint(BCLog::CJOIN, "%s CreateDenominated step 1: mtx: %s, nValue: %f, nDenom: %f\n", m_wallet->GetDisplayName(), mtx.GetHash().ToString(), (float)nValue/COIN, (float)nDenomValue/COIN);
