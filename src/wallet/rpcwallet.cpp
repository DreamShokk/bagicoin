    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The chaincoin address to be associated with a label."},
                    {"label", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The label to assign to the address."},
                }}
                .ToString() +
            "\nExamples:\n"
            + HelpExampleCli("setlabel", "\"CwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" \"tabby\"")
            + HelpExampleRpc("setlabel", "\"CwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\", \"tabby\"")
        );
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 9)
        throw std::runtime_error(
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The chaincoin address to send to."},
        ...
        throw std::runtime_error(
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The chaincoin address to use for the private key."},
            ...
            + HelpExampleCli("verifymessage", "\"CD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\" \"signature\" \"my message\"") +
            "\nAs a JSON-RPC call\n"
            + HelpExampleRpc("signmessage", "\"CD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\", \"my message\"")
        );
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The chaincoin address for transactions."},
            ...
            "\nAs a JSON-RPC call\n"
            + HelpExampleRpc("getreceivedbyaddress", "\"CD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\", 6")
       );
            ...
            RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, /* opt */ false, /* default_val */ "", "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, /* opt */ false, /* default_val */ "", "The chaincoin address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
...
static UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg =
            RPCHelpMan{"addmultisigaddress",
                ...
                "If 'label' is specified, assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, /* opt */ false, /* default_val */ "", "A json array of bitcoin addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "chaincoin address or hex-encoded public key"},
                        },
                        },
            ...
            + HelpExampleCli("addmultisigaddress", "2 \"[\\\"C6sSauSf5pF2UkUwvKGq4qjNRzBZYqgEL5\\\",\\\"171sgjn4YtPu27adkKGrdDwzRTxnRkBfKV\\\"]\"") +
            "\nAs a JSON-RPC call\n"
            + HelpExampleRpc("addmultisigaddress", "2, \"[\\\"C6sSauSf5pF2UkUwvKGq4qjNRzBZYqgEL5\\\",\\\"171sgjn4YtPu27adkKGrdDwzRTxnRkBfKV\\\"]\"")
        ;
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"address\",    (string) The chaincoin address of the transaction.\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"keypoolrefill",
                "\nFills the keypool."+
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"newsize", RPCArg::Type::NUM, /* opt */ true, /* default_val */ "1000", "The new keypool size"},
                }}
                .ToString() +
            "\nExamples:\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending bitcoins\n",
                {
                    ...
                    {"mixingonly", RPCArg::Type::BOOL, /* opt */ true, /* default_val */ "UNSET", "Disable sending functions"},
                }}
                .ToString() +
            "\nNote:\n"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Chaincoin address: ") + input.get_str());
            }
    ...
    std::vector<COutput> vecOutputs;
    {
        auto locked_chain = pwallet->chain().lock();
        LOCK(pwallet->cs_wallet);
        pwallet->AvailableCoins(*locked_chain, vecOutputs, !include_unsafe, nullptr, ALL_COINS, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth);
    }
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid chaincoin address");
            }
            ...
            RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, /* opt */ true, /* default_val */ "null", "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* opt */ true, /* default_val */ "pool address", "The chaincoin address to receive the change"},
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"getaddressinfo",
                "\nReturn information about the given chaincoin address. Some information requires the address\n"
                "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The chaincoin address to get the information of."},
                }}
                .ToString() +
            "\nResult:\n"
            "{\n"
            "  \"address\" : \"address\",        (string) The chaincoin address validated\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"walletprocesspsbt",
                ...
                "that we can sign for." +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"psct", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The transaction base64 string"},
...
UniValue getmnoutputs(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getmnoutputs\n"
    ...
    std::vector<COutput> vecOutputs;
    {
        auto locked_chain = pwallet->chain().lock();
        LOCK(pwallet->cs_wallet);
        pwallet->AvailableCoins(*locked_chain, vecOutputs, true, nullptr, ONLY_1000);
    }
...
{ //  category              name                                actor (function)                argNames
    ...
    { "wallet",             "getmnoutputs",                     &getmnoutputs,                  {} },
    { "wallet",             "getpoolinfo",                      &getpoolinfo,                   {} },
